...
export default class Cancelbooking extends Component {
    ...
    componentDidMount() {
        ...
        get('/booking/adminBooking/3')
            .then(res => {
                var tdata = res.data.filter(e => e.bookingDetails.orderStatus == 9)
    ...
    handleChange = e => {
        ...
        post('/booking/searchBooking/3', data)
            .then(res => {
                var tdata = res.data.filter(e => e.bookingDetails.orderStatus == 9)
                var slice = tdata.slice(this.state.offset, this.state.offset + this.state.perPage)
                this.setState({
    ...
    bookingShow = () => {
        return this.state.tableData.map((obj, i) => {
            if (obj.bookingDetails.orderStatus == 9) {
                return (
                    <tr className="odd" key={i}>
                        ...
                        <td>
                            <Link to={"/Viewbooking?booking_id=" + obj.bookingDetails.OrderID + "&status=3"} className="btn btn-icon btn-sm btn-primary mt-1 mb-1" type="button">
        ...
        return (
            <div>
                <div className="page-header mt-0 p-3">
                    <h3 className="mb-sm-0">Cancel Booking</h3>
