...
export default class Pendingreturn extends Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
        ...
        get('/booking/adminBooking/4')
            .then(res => {
                var tdatas = res.data.filter(e => e.bookingDetails.orderStatus == 5 || e.bookingDetails.orderStatus == 6 || e.bookingDetails.orderStatus == 7)
                var tdata = tdatas.sort((a, b) => b.bookingDetails.id - a.bookingDetails.id)
    ...
    handleChange = e => {
        ...
        post('/booking/searchBooking/', data)
            .then(res => {
                var tdata = res.data.filter(e => e.bookingDetails.orderStatus == 8)
                var slice = tdata.slice(this.state.offset, this.state.offset + this.state.perPage)
                this.setState({
                    bookings: res.data,
                    pageCount: Math.ceil(tdata.length / this.state.perPage),
    ...
    bookingShow = () => {
        return this.state.tableData.map((obj, i) => {
            if (obj.bookingDetails.orderStatus == 5 || obj.bookingDetails.orderStatus == 6 || obj.bookingDetails.orderStatus == 7) {
                return (
        ...
        return (
            <div>
                <div className="page-header mt-0 p-3">
                    <h3 className="mb-sm-0">Pending Return Booking</h3>
                    ...
                    <div className="col-md-12">
                        <div className="card shadow">
                            <div className="card-header">
                                <h2 className="mb-0">Pending Return Booking</h2>
                                {/* <div
                                    style={{
                                        float: "right",
                                        marginBottom: "13px",
